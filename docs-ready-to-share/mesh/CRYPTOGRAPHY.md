...
WireGuard uses similar cryptographic primitives (ChaCha20-Poly1305, Curve25519) but serves a different purpose:

| Property | WireGuard | OmertaMesh |
|----------|-----------|------------|
| Purpose | Point-to-point VPN | Mesh network coordination |
| Topology | Static tunnels | Dynamic peer discovery |
| NAT traversal | External/manual | Built-in (hole punching + relay) |
| Roaming | Requires reconfiguration | Seamless reconnection |
| Multi-peer | Separate tunnel per peer | Shared network key |

OmertaMesh uses WireGuard for the VPN layer (compute traffic) but provides mesh coordination that WireGuard doesn't address.

### vs. Meshtastic

Meshtastic is a LoRa mesh network for off-grid communication. Key cryptographic differences:

| Property | Meshtastic | OmertaMesh |
|----------|------------|------------|
| Header encryption | Unencrypted | Fully encrypted |
| Visible to observers | Source, dest, hop count, packet type | Only magic bytes (5 bytes) |
| Network identification | Visible channel | Hidden (networkHash encrypted) |
| Cross-network relay | Yes (blind relay) | No (must be member) |

**Why this matters:**

1. **Metadata protection**: OmertaMesh hides who's communicating with whom
2. **Network isolation**: Multiple networks on same infrastructure are indistinguishable
3. **Traffic analysis resistance**: Can't correlate packets by header fields
4. **Swarm privacy**: Observers can't tell how many networks exist or their membership

Meshtastic's unencrypted headers enable resilient relay through non-member nodes—valuable for emergency/off-grid scenarios. OmertaMesh prioritizes privacy for compute swarms where hiding participation and communication patterns matters.
